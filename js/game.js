...
function moveBear(e) {
    ...
    if (e.keyCode == KEYRIGHT) {
        bear.move(bear.speed, 0)
    } // right key
    if (e.keyCode == KEYLEFT) {
        bear.move(-bear.speed, 0)
    } // left key
    if (e.keyCode == KEYUP) {
        bear.move(0, -bear.speed)
    } // up key
...
//changing the speed of the bear
function setspeed() {
    bear.speed = document.getElementById("bearspeed").value;
}
...
class Bee {
    constructor(beeNumber) {
        ...
        this.display = function () {
            ...
            this.htmlElement.style.left = this.x + "px";
            this.htmlElement.style.top = this.y + "px";
            this.htmlElement.style.position = "block";
        };
        this.fitBounds = function () {
...
function createBeeImg(wNum) {
    ...
    console.log(boardDivW, boardDivH);
...
function updateBees() { // update loop for game
    ...
    let period = document.getElementById("periodTimer").value || 10;//modify this to control refresh period
    //update the timer for the next move
    updateTimer = setTimeout('updateBees()', period);
}


//Checking intersection between bear and bees
...
function isHit(defender, offender) {
    if (overlap(defender, offender)) { //check if the two image overlap
        ...
        //Calculate the longest duration between stings
        ...
        document.getElementById("duration").innerHTML = longestDuration ? longestDuration : "?";
    }
}
